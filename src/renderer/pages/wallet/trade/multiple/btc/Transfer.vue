    ...
    methods: {
      ...
      sign() {
        if (!this.to || !this.amount) {
          ...
          this.$router.push(`/wallet/transfermul/btc/pwd?address=${this.address}&coin=${this.coin}&to=${this.to}&amount=${this.amount}&first=${this.first}&isapply=${this.isapply}&pendingId=${this.pendingId}&fee=${this.fee}`);
        }
      },
      process() {
